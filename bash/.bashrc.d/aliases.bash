...
# gbr: remote branches (last 100 by recency) with tip author; run gf first to refresh
alias gbr='git branch -r --sort=committerdate --format="%(refname:short)%09%(authorname)%09%(committerdate:relative)" | grep -v "/HEAD" | tail -100 | awk -F"\t" "{ printf \"%-46s %-20s (%s)\\n\", \$1, \$2, \$3 }"'
