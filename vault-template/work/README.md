# Work Notes Vault

A plain-markdown work knowledge vault, edited in Neovim via
[obsidian.nvim](https://github.com/obsidian-nvim/obsidian.nvim) (Obsidian-compatible). **Private** - synced to a private
GitLab repo.

## Structure - the PARA method

PARA organizes notes by **how actionable they are right now, not by topic**. That single idea is the whole method:

| Folder       | What it holds                      | Test                                            |
| ------------ | ---------------------------------- | ----------------------------------------------- |
| `projects/`  | a goal with **an end**             | "Does it finish?" - ship a service, a migration |
| `areas/`     | a **standard to maintain**, no end | "Keep it up forever?" - on-call, a team ritual  |
| `resources/` | **reference** / topics of interest | "Want it later?" - runbooks, gcp notes          |
| `archive/`   | **done or dormant** items          | "Inactive?"                                     |

Supporting folders (the mechanics, not PARA):

- `inbox/` - where new notes land; triage into PARA later
- `dailies/` - one note per day (`YYYY-MM-DD.md`)
- `templates/` - note templates
- `assets/` - pasted images and attachments
- `Home.md` - Map of Content; the hand-curated index / front door
- `<bucket>/index.md` - regenerated from frontmatter by `/index`; never hand-edited

Folders answer _where a note belongs_; `#tags` answer _what it is about_; `[[links]]` answer _how it connects_ - use all
three rather than forcing everything into folders.

## Frontmatter

Every filed note carries YAML frontmatter (`type` required; `title`, `status`, `tags`, `created`, `updated`, `summary`
recommended). It keeps the vault greppable and lets `/index` rebuild the maps without reading prose. Full schema and the
filing decision tree live in [`CLAUDE.md`](CLAUDE.md).

## Tasks

Tasks live **with their context**, not in one big list: a project's to-dos go in its project note, quick captures in
today's daily note. Use plain `- [ ]` checkboxes (toggle with `<CR>` in obsidian.nvim), and scan/clear them during the
weekly review.

## Workflow

1. **Capture** everything into `inbox/` or today's daily note - zero decisions, zero friction.
2. **Link liberally** with `[[wiki-links]]`; linking to a note that does not exist yet is fine.
3. **Weekly review** (the highest-value habit): empty `inbox/`, file notes into PARA, archive what is finished.
4. **Permanent notes**: durable, reusable ideas become atomic notes in `resources/`, titled as a claim.

## Agent layer

This vault is set up for Claude Code. Slash commands (`.claude/commands/`) automate the chores, proposal-first:
`/index`, `/inbox-triage`, `/weekly-review`, `/distill`, `/link-audit`. Guardrail hooks (`.claude/`, `.githooks/`)
enforce the hard rules - no file access outside this vault, no committing secrets. See [`CLAUDE.md`](CLAUDE.md) for the
details.

## Privacy

Work and company material only. The repo stays **private**; no personal content (that belongs in the separate personal
vault, `~/vaults/personal`) and no secrets or credentials.
