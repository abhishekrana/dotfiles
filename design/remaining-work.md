...
| bat / env (item D)            | ✅ switcher writes `~/.config/theme/env.sh` (`BAT_THEME`); `bash/.bashrc.d/theme.bash` sources it. Solarized works now; Catppuccin needs its `.tmTheme` installed (item H) — **unpushed** |
