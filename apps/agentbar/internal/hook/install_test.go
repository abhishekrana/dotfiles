...
func TestInstallCreatesAndIsIdempotent(t *testing.T) {
	...
	changed, err := Install(path, "/opt/bin/agentbar")
	if err != nil || !changed {
...
func TestInstallMigratesOldStyleEntries(t *testing.T) {
	...
	old := `{
	  "hooks": {
	    "Stop": [
	      ...
	      {"hooks": [{"type": "command", "command": "$HOME/.tmux/plugins/agentbar/bin/agentbar hook"}]}
	    ]
	  }
	}`
	...
	if _, err := Install(path, "/opt/bin/agentbar"); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(last, `[ -x "/opt/bin/agentbar" ]`) {
