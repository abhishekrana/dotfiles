...
ci_color() {                                             # pipeline status -> fg style
  case "$1" in
    ...
    pending|created|waiting_for_resource|scheduled|manual)
                                                printf '#[fg=#b58900]' ;;  # yellow
...
cache_get() {
  ...
  while IFS='=' read -r k v; do [ "$k" = "$want" ] && { printf '%s' "$v"; return; }; done < "$file"
}
...
cmd_refresh() {
  ...
  if [ -n "$prefix" ]; then
    local ij; ij=$(glab issue view "$prefix" -F json 2>/dev/null)
    if [ -n "$ij" ]; then
  ...
  if [ -z "$issue_iid" ] && [ -n "$mr_iid" ]; then
    local cj; cj=$(glab api "projects/:fullpath/merge_requests/$mr_iid/closes_issues" 2>/dev/null)
    if [ -n "$cj" ]; then
...
cmd_open() {
  ...
  [ -n "$url" ] || { tmux display-message "GitLab: no ${seg#gl-} link"; return 0; }
