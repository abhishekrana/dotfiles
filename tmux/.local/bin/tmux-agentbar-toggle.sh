...
#
# The "☰ agents" status chip calls this. Deliberately NOT the global prefix+e /
# scripts/toggle.sh, which opens/closes a sidebar in EVERY session at once (~13
# animated panes redrawing together tips this tmux+Ghostty client into a
# scroll-corruption storm). Touching one session = one split/kill = a small,
# safe redraw. Open path reuses agentbar's own open.sh; close mirrors toggle.sh's
# close_in for a single session.
