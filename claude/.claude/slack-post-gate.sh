...
payload=$(cat 2>/dev/null) || { echo '{}'; exit 0; }
command -v jq >/dev/null 2>&1 || { echo '{}'; exit 0; }
[ "$(jq -r '.tool_name // empty' <<<"$payload")" = "Bash" ] || { echo '{}'; exit 0; }
cmd=$(jq -r '.tool_input.command // empty' <<<"$payload")
grep -q 'chat\.postMessage' <<<"$cmd" || { echo '{}'; exit 0; }

allow_file=${SLACK_UNATTENDED_CHANNELS_FILE:-$HOME/.config/claude/slack-unattended-channels}
allowed=""
[ -f "$allow_file" ] && allowed=$(sed -e 's/#.*//' -e 's/[[:space:]]//g' "$allow_file" 2>/dev/null | grep -v '^$' || true)
...
for c in $allowed; do
  [ -n "$channel" ] && [ "$channel" = "$c" ] && { echo '{}'; exit 0; }
done

if [ -z "$channel" ]; then
  reason="Slack post whose target channel could not be read from the command. Confirm the channel and message before sending."
else
